...
def clear_client_instance_cache_recurisvely(superclass)
  superclass.subclasses.compact.each do |subclass|
    subclass.client = nil
    clean_client_instance_recurisvely(subclass) if subclass.subclasses
  end
end

def after_each_spaceship
  # Clear client caches that may cause dependencies among unit test cases each other
  Spaceship::Client.client = nil
